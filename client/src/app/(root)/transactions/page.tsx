  ...
  columnHelper.accessor("amount", {
    ...
    cell: (info) => {
      ...
      return (
        <p
          className={cn(
            "text-xs text-gray-500 text-left font-bold",
            !isNegative && "text-secondary-green",
          )}
        >
...
const mobileColumns = [
  ...
  columnHelper.accessor("amount", {
    cell: (info) => (
      <p className="text-xs text-gray-500 text-right">{info.renderValue()}</p>
    ),
  }),
];
...
const TransacionsPage = () => {
  ...
  const category = transactions.map(
  ...
  const handleFilterChange = (value: string) => {
    const validActions: FilterAction[] = [
      "latest",
      "oldest",
      "atoz",
      "ztoa",
  ...
  const transactionsFiltered = useMemo(() => {
    ...
    if (filterValue.latest) {
      ...
      filtered = [...filtered].sort((a: Transaction, b: Transaction) =>
            ...
            <div className="flex items-center gap-2 w-full ">
              ...
              <Select onValueChange={handleFilterChange}>
                ...
                <SelectContent className="bg-white shadow-lg rounded-md">
                  <SelectItem
                    value="latest"
                    className="text-xs text-gray-700 px-2 py-1 hover:bg-gray-100"
                  >
                    Latest
                  </SelectItem>
                  <SelectItem
